...
document.addEventListener("click", (event) => {
    ...
    if (button.classList.contains("view-user-btn")) {
        ...
        const modal = document.getElementById(`view-user-modal-${userId}`);
    ...
    if (button.classList.contains("activate-user-btn")) {
        ...
        const modal = document.getElementById(`activate-user-modal-${userId}`);
...
deactivateForms.forEach((form) => {
    form.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            const userId = form.id.replace("deactivate-user-form-", "");
            ...
            if (!response.ok) {
                if (errorMessage) {
                    errorMessage.textContent =
                        data.message || "Unable to deactivate user.";
...
activateForms.forEach((form) => {
    form.addEventListener("submit", async (event) => {
        ...
        try {
            ...
            const userId = form.id.replace("activate-user-form-", "");
...
function updateUserRow(user) {
    ...
    row.children[0].textContent = `${user.first_name} ${user.last_name}`;

    row.children[1].textContent = user.email;

    updateRoleBadge(row.children[2], user.role);

    row.children[3].textContent = formatDate(user.created_at);

    updateStatusBadge(row.children[4], user.status);

    updateActionButton(row.children[5], user);
}
...
function updateUserCard(user) {
    ...
    const nameElement = card.querySelector("[data-user-name]");

    if (nameElement) {
        nameElement.textContent = `${user.first_name} ${user.last_name}`;
    }

    const emailElement = card.querySelector("[data-user-email]");

    if (emailElement) {
        emailElement.textContent = user.email;
    }

    const roleElement = card.querySelector("[data-user-role]");

    if (roleElement) {
        updateRoleElement(roleElement, user.role);
    }

    const statusElement = card.querySelector("[data-user-status]");

    if (statusElement) {
        updateStatusElement(statusElement, user.status);
    }

    updateMobileActionButton(card, user);
}

function updateRoleBadge(cell, role) {
    const badge = cell.querySelector("[data-user-role]");

    if (!badge) {
        return;
    }

    updateRoleElement(badge, role);
}
...
function updateRoleElement(element, role) {
    ...
    if (role === "admin") {
        element.classList.add("bg-purple-100", "text-purple-700");
...
function updateStatusBadge(cell, status) {
    const badge = cell.querySelector("[data-user-status]");

    if (!badge) {
        return;
    }

    updateStatusElement(badge, status);
}
...
function filterUsers(page = 1) {
    ...
    const rows = userTableBody.querySelectorAll(
    ...
    const cards = userCardList
        ? userCardList.querySelectorAll("[id^='user-card-']")
        : [];
